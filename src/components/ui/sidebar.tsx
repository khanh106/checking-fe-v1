...
import { PanelLeftExpandFilled, PanelRightExpandFilled } from "@fluentui/react-icons"
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("size-8 hover:bg-transparent", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      {open ? <PanelRightExpandFilled className="size-8" /> : <PanelLeftExpandFilled className="size-8" />}
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn("flex flex-col items-end gap-2 p-4", "group-data-[state=collapsed]:items-center", className)}
      {...props}
    />
  )
}
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex min-h-0 flex-1 flex-col gap-2 overflow-auto p-4", "group-data-[collapsible=icon]:overflow-hidden", className)}
      {...props}
    />
  )
}
...
const sidebarMenuButtonVariants = cva(
  [
    // Base styles
    "peer/menu-button flex w-full items-center gap-1 overflow-hidden rounded p-2",
    ...
    "group-data-[collapsible=icon]:size-11! group-data-[collapsible=icon]:p-2!",

    // Content styling
    "[&>span:last-child]:truncate [&_svg:not([class*='size-'])]:size-6 [&>svg]:shrink-0",
  ],
  {
    variants: {
      variant: {
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn("flex flex-col gap-2.5", "group-data-[collapsible=icon]:hidden", className)}
      {...props}
    />
  )
}
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      className={cn(
        // Base styles
        "text-sidebar-foreground ring-sidebar-ring flex h-14 min-w-0 -translate-x-px",
        "items-center gap-2 overflow-hidden rounded-md p-2 pl-8 outline-hidden",
