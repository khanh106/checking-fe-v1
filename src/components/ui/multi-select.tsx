...
import { ChevronDown24Regular, DismissRegular } from "@fluentui/react-icons"
...
interface MultiSelectProps extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement>, "animationConfig"> {
  ...
  customRenderSelected?: (selectedOptions: MultiSelectOption[]) => React.ReactNode
}
        ...
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                "flex h-auto min-h-11 items-center justify-between rounded bg-inherit px-3 py-2.5 font-normal text-inherit hover:bg-inherit [&_svg]:pointer-events-auto",
                ...
                <div className="flex w-full items-center justify-between">
                  <div
                    className={cn(
                      ...
                      singleLine ? "multiselect-singleline-scroll scrollbar-thin overflow-x-auto" : "flex-wrap",
                  ...
                  >
                    {customRenderSelected ? (
                      customRenderSelected(selectedValues.map(getOptionByValue).filter(Boolean) as MultiSelectOption[])
                    ) : (
                      <>
                        {selectedValues
                          ...
                          .map((value) => {
                            ...
                            return (
                              <Badge key={value} variant="tag">
                                {IconComponent && !responsiveSettings.hideIcons && (
                                  <IconComponent
                                    className={cn("mr-2 h-4 w-4", responsiveSettings.compactMode && "mr-1 h-3 w-3", customStyle?.iconColor && "text-current")}
                                ...
                                {showRemoveIcon && (
                                  ...
                                  >
                                    <DismissRegular className={cn("size-3", responsiveSettings.compactMode && "size-2.5")} />
                                  </div>
                                )}
                              </Badge>
                            )
                  ...
                  <div className="flex items-center justify-between">
                    {showClearIcon && (
                      <>
                        ...
                        >
                          <DismissRegular className="size-6" />
                        </div>
                        <Separator orientation="vertical" className="flex h-full min-h-6" />
                      </>
                    )}
                    <ChevronDown24Regular className="text-muted-foreground mx-2 size-6 cursor-pointer" aria-hidden="true" />
                  </div>
                </div>
              ) : (
                <div className="mx-auto flex w-full items-center justify-between">
                  <span className="text-muted-foreground text-sm">{placeholder}</span>
                  <ChevronDown24Regular className="text-muted-foreground mx-2 size-6 cursor-pointer" />
                </div>
              )}
            </Button>
          </PopoverTrigger>
          ...
          >
            <Command>
              ...
              <CommandList
                className={cn("multiselect-scrollbar max-h-[40vh] overflow-y-auto", screenSize === "mobile" && "max-h-[50vh]", "overscroll-behavior-y-contain")}
              >
                      ...
                      {group.options.map((option) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {option.icon && <option.icon className="text-muted-foreground size-4" aria-hidden="true" />}
